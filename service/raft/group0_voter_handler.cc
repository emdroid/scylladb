...
future<> group0_voter_handler::update_nodes(
    ...
    auto get_replica_state = [this](const raft::server_id& id, bool allow_transient) -> const replica_state* {
        ...
        if (allow_transient) {
            // When adding a node, it may not be present in normal nodes yet (or not any more), but might be joining or leaving
            // and be in the transient state.
            // We should only consider transient nodes that are already voters, or nodes being newly added (joining).
            if (const auto it = _topology.transition_nodes.find(id); it != _topology.transition_nodes.end()) {
                return &it->second;
            }
            rvlogger.debug("Node {} is not present in the transient nodes", id);
        }
        return nullptr;
    };
    ...
    auto add_nodes_from_list = [&add_node, &get_replica_state, &group0_config](const std::set<locator::host_id>& nodes_set, bool is_alive) {
        for (const auto& host_id : nodes_set) {
            ...
            // We only allow transient nodes to be added if they are already voters.
            const bool allow_transient = group0_config.can_vote(id);
            const auto* node = get_replica_state(id, allow_transient);
            if (!node) {
