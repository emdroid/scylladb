...
async def test_raft_voters_multidc(manager: ManagerClient, num_nodes: int):
    """
    ...
    Assert:
        - test the topology didn't lose the majority by sending a read barrier request to one of the smaller DCs
    ...
    dc_setup = [
        ...
        {
            'property_file': {'dc': 'dc2', 'rack': 'rack'},
            'num_nodes': num_nodes,
        },
        {
            'property_file': {'dc': 'dc3', 'rack': 'rack'},
            'num_nodes': num_nodes,
        },
    ]
    ...
    # Note: This also tests the topology coordinator migration
    # (the coordinator will be most likely in dc1 initially, but after the nodes are killed, it should migrate to one of
    # the other DCs - testing that the voters status is transferred correcly)

    logging.info('Killing down all nodes in dc1')
